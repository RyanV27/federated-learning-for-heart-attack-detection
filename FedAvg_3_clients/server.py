...
seed_num = 30
run_num = 1
folder_names = ['ptbdb_data_cli1/', 'ptbdb_data_cli2/', 'ptbdb_data_cli3/']
...
def server_aggregate(global_model, client_models, no_of_clients):
    global_dict = global_model.state_dict()
    
    for k in global_dict.keys():
        # print(k, " shape = ", global_dict[k].shape)
        # print(global_dict[k])
        updatad_model_param = torch.zeros(global_dict[k].shape)
        for i in range(no_of_clients):
            client_dict = client_models[i].state_dict()
            updatad_model_param += client_dict[k]
            # print("\nClient ", i)
            # print(client_dict[k])
        # print("updatad_model_param = \n", updatad_model_param)

        updatad_model_param /= no_of_clients
        # print("updatad_model_param after mean = \n", updatad_model_param)

        global_dict[k] = updatad_model_param
...
def run_server():
    ...
    while True:
        ...
        global_model = server_aggregate(global_model, client_models, no_of_clients)
        print("\nGlobal Model Updatad!")
        
        # Printing accuracy of global models
        avg_accuracy =  0.0
        no_of_tests = 5
        for i in range(no_of_tests):
            print(f"Test {i}:")
            accuracy = validate_global_model(global_model, data_unhealthy_train, data_healthy_train, 
                                    data_unhealthy_val, data_healthy_val, epochs = 100, batch_size = 10)
            avg_accuracy += (accuracy / no_of_tests)
        print("Average accuracy of global model = ", avg_accuracy)
