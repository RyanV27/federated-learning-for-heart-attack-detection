...
class ConvNetQuake(nn.Module):
    def __init__(self, batch_size, local_epochs, learning_rate, learning_rate_decay_gamma):
        ...
        for param in self.parameters():
            self.control.append(torch.zeros_like(param))
        self.delta_control = []
        self.delta_y = []
...
# Function to update the control variable of client
def update_yc(global_model, model, client_c, global_c, max_local_epochs):
    for ci, c, x, yi in zip(client_c, global_c, global_model.parameters(), model.parameters()):
        # ci.data = ci - c + 1/model.batch_size/max_local_epochs/model.learning_rate * (x - yi)
        ci.data = ci - c + 1/model.batch_size/max_local_epochs/model.learning_rate * (x - yi)
    
    return(client_c)


def delta_yc(global_model, model, global_c, max_local_epochs):
    delta_y = []
    delta_c = []
    for c, x, yi in zip(global_c, global_model.parameters(), model.parameters()):
        delta_y.append(yi - x)
        # delta_c.append(- c + 1/model.num_batches/max_local_epochs/model.learning_rate * (x - yi))
        delta_c.append(- c + 1/model.batch_size/max_local_epochs/model.learning_rate * (x - yi))
...
def train_local_model(model, data_unhealthy_train, data_healthy_train, 
    ...
    optimizer = SCAFFOLDOptimizer(model.parameters(), lr = model.learning_rate)
    learning_rate_scheduler = torch.optim.lr_scheduler.ExponentialLR(
        optimizer = optimizer, 
        gamma = model.learning_rate_decay_gamma
    )
    ...
    model.control = update_yc(global_model, model, model.control, server_control, epochs)
    model.delta_y, model.delta_control = delta_yc(global_model, model, server_control, epochs)
...
def run_client():
    ...
    #    client_socket.connect(('localhost', 8080))
        ...
        # Initialize model
        model = ConvNetQuake(batch_size = 10, local_epochs = 30000, learning_rate = 0.005, learning_rate_decay_gamma = 0.99)
        ...
        if fed_iteration_no > 1:
            ...
            for param in model.parameters():
                server_control.append(torch.zeros_like(param))
        ...
        model = train_local_model(model, data_unhealthy_train, data_healthy_train, 
                                    data_unhealthy_val, data_healthy_val, server_control, lr=model.learning_rate, epochs=30000, 
                                        batch_size = 10)
    ...
    #    print("\nSent model parameters to the server.")
