...
class ConvNetQuake(nn.Module):
    def __init__(self, server_learning_rate):
        ...
        for param in self.parameters():
            ...
            self.delta_y.append(torch.zeros_like(param))
            self.delta_control.append(torch.zeros_like(param))
...
def server_aggregate(global_model, client_models, no_sample_clients, no_total_clients):
    ...
    global_c_copy = copy.deepcopy(global_model.control)
    
    for cid in range(no_sample_clients): 
        dy = client_models[cid].delta_y
        dc = client_models[cid].delta_control

        for server_param, client_param in zip(global_model_copy.parameters(), dy):
            server_param.data += client_param.data.clone() / no_sample_clients * global_model.server_learning_rate
        for server_param, client_param in zip(global_c_copy, dc):
            server_param.data += client_param.data.clone() / no_total_clients
    
    global_model = global_model_copy
    global_model.control = global_c_copy
...
def run_server():
    # Initialize model
    global_model = ConvNetQuake(server_learning_rate = 1.0)
    ...
    while True:
        ...
        #for _ in range (no_of_clients):
            ...
            model = ConvNetQuake(server_learning_rate = 1.0)
        ...
        print("Saved global model parameters in server_model_parameters.pth\n")
